...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    try {
      if (typeof window !== "undefined") {
        const savedCart = window.localStorage.getItem("vcoop_cart");
        if (savedCart) {
  ...
  useEffect(() => {
    try {
      if (typeof window !== "undefined") {
        window.localStorage.setItem("vcoop_cart", JSON.stringify(cartItems));
      }
    } catch (error) {
  ...
  const addToCart = (product) => {
    setCartItems((prevItems) => {
      ...
      if (existingItem) {
        ...
        // If new item, add with cartQuantity 1
        return [
          ...prevItems,
          {
            ...product,
